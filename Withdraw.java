...
import javax.swing.*;
import java.sql.*;
class Withdraw extends Panel implements ActionListener
{
	Label l1,l2,l3;
	TextField t1,t2,t3;
	Button b1;
	...
	Withdraw()
	{
		...
		l2=new Label("Enter Withdraw Amount :");
				...
				if((JOptionPane.showConfirmDialog(null,"Do You Want To Continue","Bank",0))==0)
				{
					try
					{
						...
						if(rs.next())
						{
							...
							st.execute("insert into transactions(id,deposit,withdraw,bal)values("+s+","+y+","+Integer.parseInt(t2.getText())+","+t+")");
							JOptionPane.showMessageDialog(null,"Amount Withdrawed","Account",0);
